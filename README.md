...
Tester for the [`Python-based NGSI-LD API client`](https://github.com/giros-dit/python-ngsi-ld-client/tree/1.7.1) compliant with the [NGSI-LD API OpenAPI specification](https://raw.githubusercontent.com/giros-dit/python-ngsi-ld-client/1.7.1/schemas/ngsi-ld-api.yaml).

To deploy a docker-compose scenario with the [`Orion-LD`](https://github.com/FIWARE/context.Orion-LD) context broker (a reference implementation of NGSI-LD context broker compliant with the  NGSI-LD API specification by [ETSI GS CIM 009 V1.7.1](https://www.etsi.org/deliver/etsi_gs/CIM/001_099/009/01.07.01_60/gs_CIM009v010701p.pdf)), execute the following command:
```bash
$ docker compose up
```
...
To validate the [`create_entity`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#create_entity), [`retrieve_entity`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationConsumptionApi.md#retrieve_entity), [`query_entity`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationConsumptionApi.md#query_entity), [`update_entity`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#update_entity), [`delete_entity`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#delete_entity), and [`upsert_batch`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#upsert_batch) `CRUD` operations for a simple NGSI-LD Entity (e.g., NGSI-LD Entity of type `IotDevice` available [here](iot-sensors-tester/examples/iot-device/example-normalized.json)), follow these steps:
...
6. To create different NGSI-LD Entities (e.g, NGSI-LD Entities of type `IotDevice`, `TemperatureSensor`, and `HumiditySensor`) by using its `id` field (i.e., `upsert_batch` operation), run the [iot-sensors-tester/upsert-iot-sensors-entities.py](iot-sensors-tester/upsert-iot-sensors-entities.py) Python script as follow:
    ...
    > **Note:**
    >
    > The NGSI-LD _unit codes_ for representing values of temperature in degrees Celsius and percent of humidity are extracted from UNECE/CEFACT Common Codes for specifying the unit of measurement [[1](http://www.unece.org/fileadmin/DAM/cefact/recommendations/rec20/rec20_Rev9e_2014.xls)], [[2](https://github.com/easy-global-market/ngsild-api-data-models/blob/master/UnitCodes.md#uncefact-common-codes)] as specified by [ETSI GS CIM 009 V1.7.1](https://www.etsi.org/deliver/etsi_gs/CIM/001_099/009/01.07.01_60/gs_CIM009v010701p.pdf).
...
>
> There are additional Python scripts for validating other OpenAPI NGSI-LD operations such as the [`update_attrs`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#update_attrs), [`append_attrs`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationProvisionApi.md#append_attrs), and [`create_subscription`](https://github.com/giros-dit/python-ngsi-ld-client/blob/1.7.1/docs/ContextInformationSubscriptionApi.md#create_subscription) but they are still under testing and development. The following instructions can be taken for now:
