...
class OffStreetParking(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be OffStreetParking.")
    ...
    name: Name
    available_spot_number: Optional[AvailableSpotNumber] = Field(default=None, alias="availableSpotNumber")
    total_spot_number: Optional[TotalSpotNumber] = Field(default=None, alias="totalSpotNumber")
    operated_by: OperatedBy = Field(alias="operatedBy")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "name", "availableSpotNumber", "totalSpotNumber", "operatedBy"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OffStreetParking']):
            raise ValueError("must be one of enum values ('OffStreetParking')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of available_spot_number
        if self.available_spot_number:
            _dict['availableSpotNumber'] = self.available_spot_number.to_dict()
        # override the default output from pydantic by calling `to_dict()` of total_spot_number
        if self.total_spot_number:
            _dict['totalSpotNumber'] = self.total_spot_number.to_dict()
        # override the default output from pydantic by calling `to_dict()` of operated_by
        if self.operated_by:
            _dict['operatedBy'] = self.operated_by.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'OffStreetParking',
            ...
            "availableSpotNumber": AvailableSpotNumber.from_dict(obj["availableSpotNumber"]) if obj.get("availableSpotNumber") is not None else None,
            "totalSpotNumber": TotalSpotNumber.from_dict(obj["totalSpotNumber"]) if obj.get("totalSpotNumber") is not None else None,
            "operatedBy": OperatedBy.from_dict(obj["operatedBy"]) if obj.get("operatedBy") is not None else None
        })
